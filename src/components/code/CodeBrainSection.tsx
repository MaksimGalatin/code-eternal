...
export default function CodeBrainSection() {
  ...
  return (
    <section id="code-brain" className="relative py-24 md:py-32 hex-grid grid-dissolve" ref={ref}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8, delay: 0.2 }}
          className="glass-card glass-card-cyber corner-brackets rounded-2xl p-6 md:p-10 mb-16 scan-overlay">
          <div className="flex flex-wrap items-center justify-center gap-3 mb-8">
            {["Obsidian", "Ollama Cloud", "Arweave", "WSL2", "Docker", "AI Agents"].map((tech) => (
              <span key={tech} className="px-3 py-1.5 text-xs font-mono rounded-full border border-cyan-400/20 bg-cyan-400/5 text-cyan-400 animated-underline chip-hover">{tech}</span>
            ))}
          </div>
            ...
            {[
              ...
              <div key={item.title} className="text-center">
                <div className="w-14 h-14 mx-auto rounded-xl bg-cyan-400/10 flex items-center justify-center mb-4 icon-container-glow">
                  <item.icon size={24} className="text-cyan-400 icon-spin-slow" />
                </div>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8, delay: 0.4 }}>
          <h3 className="text-xl md:text-2xl font-bold text-center mb-12 badge-glow">
              ...
              {TIMELINE_KEYS.map((key, i) => {
                ...
                return (
                  <motion.div key={key} initial={{ opacity: 0, x: isLeft ? -20 : 20 }} animate={isInView ? { opacity: 1, x: 0 } : {}}
                    ...
                    <div className="absolute left-4 md:left-1/2 top-4 w-2 h-2 rounded-full bg-cyan-400 glow-cyan-strong timeline-dot-pulse z-10 -translate-x-1 md:-translate-x-1 glow-ring" />
